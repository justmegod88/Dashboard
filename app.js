...
(function () {
  ...
  const rate = v => v == null ? '데이터 없음' : `${Number(v) >= 0 ? '+' : ''}${Number(v).toFixed(1)}%p`;
  const pp = v => v == null ? '데이터 없음' : `${Number(v) >= 0 ? '+' : ''}${Number(v).toFixed(1)}%p`;
  const pct = v => v == null ? '데이터 없음' : `${Math.round(v * 100)}%`;
  const pack = v => v == null ? '데이터 없음' : `${Math.round(Number(v)) >= 0 ? '+' : ''}${Math.round(Number(v)).toLocaleString('ko-KR')}팩`;
  ...
  function metrics(id) {
    ...
    const growths = {
      ast: { rate: fittingGrowthRate(sr, 'ast'), pack: fittingPackDelta(sr, 'ast') },
      mf: { rate: fittingGrowthRate(sr, 'mf'), pack: fittingPackDelta(sr, 'mf') },
      max: { rate: fittingGrowthRate(sr, 'max'), pack: fittingPackDelta(sr, 'max') }
    };
  ...
  function productPerception(m, key) {
    const vals = m.perc.filter(p => p.제품군 === key).map(p => p.점수).filter(v => v != null && Number.isFinite(Number(v)));
    return vals.length ? vals.reduce((a, b) => a + Number(b), 0) / vals.length : null;
  }

  function overallGrowth(key) {
    return fittingGrowthRate(S.sales, key);
  }
  ...
  function kpiGrowth(key, rows) {
    ...
    return kpi(
      title,
      `<span class="${cls(packDelta)}">${pack(packDelta)}</span>`,
      `<span>${rate(current)} <span class="kpi-sub">(vs PY)</span></span><br><span class="delta ${cls(diff)}">${pp(diff)} <span class="kpi-sub">(vs 전체평균)</span></span>`
    );
  }
  ...
  function render() {
    ...
    if ($('gapCards')) {
      const cards = [
        ...
        ['sales ast', '난시 역성장', ms.filter(m => m.growths.ast.rate != null && m.growths.ast.rate < 0).length],
        ['sales mf', '멀티포컬 역성장', ms.filter(m => m.growths.mf.rate != null && m.growths.mf.rate < 0).length],
        ['sales max', 'MAX 역성장', ms.filter(m => m.growths.max.rate != null && m.growths.max.rate < 0).length]
      ];
  ...
  function renderSegment(rows, ms) {
    ...
    $('segmentTable').innerHTML = ms.map(m => {
      ...
      return `<tr data-id="${esc(p.안경사ID)}"><td><b>${esc(p.안경사명)}</b><small><br>${esc(p.안경사ID)}</small></td><td>${esc(p.안경원명)}<small><br>${esc(p.지역)} · ${esc(p.채널)}</small></td><td>${esc(p.연차)} / ${esc(p.Tier)}</td><td>${m.eduRate == null ? '데이터 없음' : pct(m.eduRate)}</td><td>${m.gaps.length}개</td><td>${pack(m.growths.ast.pack)}<br><small>${rate(m.growths.ast.rate)}</small></td><td>${pack(m.growths.mf.pack)}<br><small>${rate(m.growths.mf.rate)}</small></td><td>${pack(m.growths.max.pack)}<br><small>${rate(m.growths.max.rate)}</small></td><td>${esc(eduName || '없음')}</td><td>${m.priority}</td></tr>`;
    }).join('');
  ...
  function lowQuestions(keys, ms, threshold = -0.5) {
    const ids = new Set(ms.map(m => m.p?.안경사ID).filter(Boolean));
    const allQ = {}, segQ = {};
    ...
    return Object.keys(segQ).map(q => ({ q, seg: avg(segQ[q]), all: avg(allQ[q]) }))
      .map(x => ({ ...x, diff: x.seg != null && x.all != null ? x.seg - x.all : null }))
      .filter(x => x.diff != null && x.diff <= threshold)
  ...
  function educationSummaryForRows(rows, key) {
    const ids = new Set(rows.map(r => r.안경사ID));
    const kws = key === 'ast' ? ['난시', '토릭', 'ASD'] : key === 'mf' ? ['멀티포컬', '다초점', '노안', 'MF'] : ['MAX', '맥스', '블루라이트', '실리콘'];
  ...
  function generateInsights() {
    ...
    Object.entries(by(all, '지역')).forEach(([region, rs]) => {
      Object.entries(by(rs, '연차')).forEach(([yr, rows]) => {
        if (rows.length >= 3) groups.push({ name: `${region} / ${yr}`, rows, dim: '지역+연차' });
      });
    });
    groups.forEach(g => {
  ...
  function renderInsights() {
    ...
    if ($('insightSummary')) $('insightSummary').innerHTML = [
      ...
      kpi('교육 로드맵', S.insights.filter(i => i.step1 && i.step2).length, 'STEP1+STEP2')
    ].join('');
  ...
  function download() {
    ...
    const rows = S.filtered.map(p => {
      ...
      return {
        ...
        난시성장률: m.growths.ast.rate,
        멀티포컬성장팩: m.growths.mf.pack,
        멀티포컬성장률: m.growths.mf.rate,
        MAX성장팩: m.growths.max.pack,
        MAX성장률: m.growths.max.rate,
        인식Gap: m.gaps.length,
        교육완료율: m.eduRate
      };
    });
  ...
  function seed() {
    ...
    S.sales = [
      { 안경사ID: 'A001', 안경원코드: 'S001', 안경원명: '데모안경원', '25년 난시 팩수': 100, '26년 난시 팩수': 120, '난시 성장률': 20, '25년 멀티포컬  팩수': 50, '26년 멀티포컬  팩수': 40, '멀티포컬 성장률': -20, '25년 MAX  팩수': 200, '26년 MAX  팩수': 260, 'MAX 성장률': 30 },
      { 안경사ID: 'A002', 안경원코드: 'S002', 안경원명: '테스트안경원', '25년 난시 팩수': 80, '26년 난시 팩수': 70, '난시 성장률': -12.5, '25년 멀티포컬  팩수': 40, '26년 멀티포컬  팩수': 60, '멀티포컬 성장률': 50, '25년 MAX  팩수': 100, '26년 MAX  팩수': 90, 'MAX 성장률': -10 }
    ];
    rebuildIndexes();
    buildFilters();
    render();
