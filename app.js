...
function storeKey(code,name){code=clean(code);name=clean(name);return code&&name?`${norm(code)}|${norm(name)}`:''}
function masterStoreKey(m){return m?storeKey(m.안경원코드,m.안경원명):''}
function salesStoreKey(r){return storeKey(get(r,['안경원코드','매장코드','거래처코드','ShipTo','SoldTo']),get(r,['안경원명','안경원','매장명','거래처명']))}
function salesRows(id){const m=S.master.find(x=>x.안경사ID===id),key=masterStoreKey(m);if(!key)return[];const seen=new Set();return S.sales.filter(r=>salesStoreKey(r)===key).filter(r=>{const sig=[key,norm(pName(r)),y25(r)??'',y26(r)??''].join('|');if(seen.has(sig))return false;seen.add(sig);return true})}
...
function salesResult(rows,key){const px=rows.filter(r=>P[key].match(pName(r)));let py=0,cy=0,has=false;px.forEach(r=>{const a=y25(r),b=y26(r);if(a!=null||b!=null){py+=a||0;cy+=b||0;has=true}});if(!has)return{py:null,cy:null,growth:null,status:'데이터 없음'};if(py===0&&cy>0)return{py,cy,growth:null,status:'신규판매'};if(py===0&&cy===0)return{py,cy,growth:null,status:'판매 없음'};return{py,cy,growth:(cy-py)/py*100,status:cy<py?'판매 감소':cy>py?'판매 증가':'유지'}}
function growth(rows,key){return salesResult(rows,key).growth}
function uniqueStores(rows){const seen=new Set();return rows.filter(r=>{const k=masterStoreKey(r);if(!k||seen.has(k))return false;seen.add(k);return true})}
function groupSales(rows,key){const stores=uniqueStores(rows);let py=0,cy=0,has=false,newSales=0;stores.forEach(m=>{const x=salesResult(salesRows(m.안경사ID),key);if(x.py!=null||x.cy!=null){py+=x.py||0;cy+=x.cy||0;has=true;if(x.status==='신규판매')newSales++}});return{py:has?py:null,cy:has?cy:null,growth:has&&py>0?(cy-py)/py*100:null,newSales,stores:stores.length}}
...
const C=new Map();function metrics(id){if(C.has(id))return C.get(id);const p=S.master.find(x=>x.안경사ID===id);const sr=salesRows(id);const perc=S.per.filter(x=>x.안경사ID===id);const gaps=perc.filter(x=>x.gap);const edu=S.edu.filter(r=>clean(get(r,['안경사ID','ID']))===id);const eduRate=edu.length?edu.filter(eduDone).length/edu.length:null;const rec=S.rec.find(r=>clean(get(r,['안경사ID','ID']))===id)||{};const sales={ast:salesResult(sr,'ast'),mf:salesResult(sr,'mf'),max:salesResult(sr,'max')};const growths={ast:{cur:sales.ast.growth},mf:{cur:sales.mf.growth},max:{cur:sales.max.growth}};const reasons=[];if(Object.values(sales).some(x=>x.status==='판매 감소'))reasons.push('판매 감소');if(gaps.length)reasons.push('인식 Gap');if(eduRate==null||eduRate<1)reasons.push('교육 미이수');const priority=reasons.length>=2?'높음':reasons.length===1?'중간':'낮음';const m={p,perc,gaps,eduRate,rec,sales,growths,reasons,priority};C.set(id,m);return m}
...
function kpiGrowth(key,rows){const x=groupSales(rows,key),all=groupSales(S.master,key);const d=x.growth!=null&&all.growth!=null?x.growth-all.growth:null;const value=x.cy==null?'데이터 없음':`${x.cy.toLocaleString('ko-KR')}팩 <span class="kpi-sub">(${x.status==='신규판매'?'신규판매':rate(x.growth)})</span>`;return kpi(`${P[key].label} 판매`,value,`<span class="delta ${dclass(d)}">${d==null?'전체 비교 없음':pp(d)+' (vs 전체)'}</span>`)}
...
function saleCell(x){if(!x||x.cy==null)return'데이터 없음';return`${x.cy.toLocaleString('ko-KR')}팩 <small>(${x.status==='신규판매'?'신규판매':rate(x.growth)})</small>`}
function renderSegment(rows,ms){$('resultCount').textContent=`${rows.length.toLocaleString('ko-KR')}명`;
  const ast=groupSales(rows,'ast'),mf=groupSales(rows,'mf'),max=groupSales(rows,'max');
  $('segmentSummary').innerHTML=`<div class="three-col"><div>${kpi('난시 판매',ast.cy==null?'데이터 없음':ast.cy.toLocaleString('ko-KR')+'팩',ast.py===0&&ast.cy>0?'신규판매':rate(ast.growth))}</div><div>${kpi('멀티포컬 판매',mf.cy==null?'데이터 없음':mf.cy.toLocaleString('ko-KR')+'팩',mf.py===0&&mf.cy>0?'신규판매':rate(mf.growth))}</div><div>${kpi('MAX 판매',max.cy==null?'데이터 없음':max.cy.toLocaleString('ko-KR')+'팩',max.py===0&&max.cy>0?'신규판매':rate(max.growth))}</div></div>`;
  $('segmentTable').innerHTML=ms.map(m=>{const p=m.p||{};const eduName=clean(get(m.rec,['추천교육명','교육명']))||contentName(get(m.rec,['추천교육ID','교육ID']));return`<tr data-id="${esc(p.안경사ID)}"><td><b>${esc(p.안경사명)}</b><small><br>${esc(p.안경사ID)}</small></td><td>${esc(p.안경원명)}<small><br>코드 ${esc(p.안경원코드||'없음')} · ${esc(p.지역)} · ${esc(p.채널)}</small></td><td>${esc(p.연차)} / ${esc(p.Tier)}</td><td>${m.eduRate==null?'데이터 없음':pct(m.eduRate)}</td><td>${m.gaps.length}개</td><td>${saleCell(m.sales.ast)}</td><td>${saleCell(m.sales.mf)}</td><td>${saleCell(m.sales.max)}</td><td>${esc(eduName||'없음')}</td><td>${esc(m.reasons.join(' · ')||'해당 없음')}</td></tr>`}).join('');document.querySelectorAll('#segmentTable tr').forEach(tr=>tr.onclick=()=>showProfile(tr.dataset.id))}
...
function renderInsightPlaceholder(){S.insights=[];$('insightSummary').innerHTML=[kpi('인사이트 상태','대기','자동 인사이트 생성 버튼을 눌러 계산하세요')].join('');$('insightCards').innerHTML='<div class="empty-state">엑셀 업로드 후 <b>자동 인사이트 생성</b> 버튼을 누르면 판매 저하 → 인식 원인 → 교육 제안 순서로 자동 분석합니다.</div>'}
function renderInsights(){S.insights=generateInsights();$('insightSummary').innerHTML=[kpi('발견 인사이트',S.insights.length,'TOP 8'),kpi('우선 대상',S.insights.reduce((a,i)=>a+i.size,0).toLocaleString('ko-KR'),'중복 포함'),kpi('인식×판매',S.insights.filter(i=>/인식/.test(i.type)).length,'원인 후보'),kpi('교육 로드맵',S.insights.filter(i=>i.step1&&i.step2).length,'STEP1+STEP2')].join('');$('insightCards').innerHTML=S.insights.length?S.insights.map((i,idx)=>`<div class="insight-card"><div class="type">${esc(i.type)}</div><h3>${idx+1}. ${esc(i.title)}</h3><div class="insight-steps"><div class="insight-step"><small>1. 증상</small>${esc(i.symptom)}</div><div class="insight-step"><small>2. 원인 후보</small>${esc(i.cause)}</div><div class="insight-step"><small>3. 교육 이력</small>${esc(i.education||'교육 이력 데이터 없음')}</div><div class="insight-step"><small>4. STEP1 교육</small>${esc(i.step1||i.action)}</div><div class="insight-step"><small>5. STEP2 교육</small>${esc(i.step2||'후속 심화 교육')}</div><div class="insight-step"><small>6. F/U KPI</small>${esc(i.followup||'4주 후 성장률/인식/교육완료율 확인')}</div></div><div class="note">개선 필요 기준: 판매 감소 · 인식 Gap · 교육 미이수</div><div class="insight-actions"><button class="button primary" data-insight="${idx}">대상 보기</button><button class="button" data-detail="${idx}">상세 보기</button></div></div>`).join(''):'<div class="empty-state">아직 표시할 자동 인사이트가 없습니다. 엑셀 업로드 후 다시 확인하세요.</div>';document.querySelectorAll('[data-insight]').forEach(b=>b.onclick=()=>{const ins=S.insights[+b.dataset.insight];S.targetIds=new Set(ins.targetIds);S.query='';render();$('queryExplanation').textContent=`인사이트 대상 필터 적용: ${ins.title} / 결과 ${S.filtered.length}명`;view('segment')});document.querySelectorAll('[data-detail]').forEach(b=>b.onclick=()=>showInsightDetail(+b.dataset.detail))}
