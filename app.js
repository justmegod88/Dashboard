(function () {
  ...
  function sheet(wb, names) {
    ...
    return name && wb.Sheets[name] ? XLSX.utils.sheet_to_json(wb.Sheets[name], { defval: '', raw: true, blankrows: false }) : [];
  }
  ...
  function avgPackDeltaPerAcc(rows, key) { const unique = dedupeSalesRows(rows); if (!unique.length) return null; const total = packDelta(unique, key); return total == null ? null : total / unique.length; }
  ...
  function uniqueValues(field) { return [...new Set(S.master.map(r => clean(r[field])).filter(Boolean))].sort((a, b) => b.length - a.length); }
  function queryHas(text, value) { return norm(text).includes(norm(value)); }
  function productFromQuery(q) { if (/멀티포컬|다초점|노안|\bMF\b/i.test(q)) return 'mf'; if (/MAX|맥스|블루라이트|눈건강/i.test(q)) return 'max'; if (/난시|토릭|ASD/i.test(q)) return 'ast'; return null; }
  function lowPerceptionForPerson(id, productKey) { const rows = S.perById.get(id) || []; return rows.some(p => (!productKey || questionRelevance(p, productKey) > 0) && (p.gap || (p.점수 != null && p.목표값 != null && p.점수 < p.목표값))); }
  function matchYearsQuery(row, q) { const m = q.match(/(\d+)\s*년차/); if (m) return clean(row.연차).includes(m[1]); const exact = uniqueValues('연차').find(v => queryHas(q, v)); return exact ? clean(row.연차) === exact : true; }
  function parseSmartConditions(q) {
    const product = productFromQuery(q);
    const conditions = [];
    const region = uniqueValues('지역').find(v => queryHas(q, v)); if (region) conditions.push({ label: `지역=${region}`, test: r => clean(r.지역) === region });
    const tier = uniqueValues('Tier').find(v => queryHas(q, v)) || (/TOP|탑|상위/i.test(q) ? uniqueValues('Tier').find(v => /top|탑|상위/i.test(v)) : ''); if (tier) conditions.push({ label: `Tier=${tier}`, test: r => clean(r.Tier) === tier });
    const channel = uniqueValues('채널').find(v => queryHas(q, v)); if (channel) conditions.push({ label: `채널=${channel}`, test: r => clean(r.채널) === channel });
    if (/(\d+)\s*년차/.test(q) || uniqueValues('연차').some(v => queryHas(q, v))) conditions.push({ label: '연차 조건', test: r => matchYearsQuery(r, q) });
    if (/인식|gap|갭|낮|부족|저하/i.test(q)) conditions.push({ label: product ? `${FITTING_COLUMNS[product].label} 인식 낮음` : '인식 낮음', test: r => lowPerceptionForPerson(r.안경사ID, product) });
    if (/교육.*미완료|미완료|미수료|미이수/i.test(q)) conditions.push({ label: '교육 미완료', test: r => { const m = metrics(r.안경사ID); return m.eduRate == null || m.eduRate < 1; } });
    if (/역성장|성장률.*음수|마이너스|성장.*낮/i.test(q)) conditions.push({ label: product ? `${FITTING_COLUMNS[product].label} 역성장` : '역성장', test: r => { const m = metrics(r.안경사ID); if (product) return m.growths[product].cur != null && m.growths[product].cur < 0; return ['ast','mf','max'].some(k => m.growths[k].cur != null && m.growths[k].cur < 0); } });
  ...
  function filtered() {
    ...
    if (q) {
      ...
      if (parsed.conditions.length) rows = rows.filter(r => parsed.conditions.every(c => c.test(r)));
        ...
        rows = rows.filter(r => Object.values(r).some(v => norm(v).includes(norm(q))));
      }
      if ($('queryExplanation')) {
  ...
  function questionRelevance(q, key) { const text = `${q.문항 || ''} ${q.문항ID || ''}`; let scoreValue = q.제품군 === key ? 4 : 0; INSIGHT[key].keywords.forEach(k => { if (text.toLowerCase().includes(String(k).toLowerCase())) scoreValue += 1; }); return scoreValue; }
  function lowQuestionsForRows(masterRows, key, maxCount = 3, relevantOnly = true) { const ids = new Set(masterRows.map(r => r.안경사ID)); const allByQ = new Map(); const segByQ = new Map(); const qInfo = new Map(); S.per.forEach(p => { const rel = questionRelevance(p, key); if (relevantOnly && rel <= 0) return; const q = p.문항; if (!q) return; if (!allByQ.has(q)) allByQ.set(q, []); allByQ.get(q).push(p.점수); qInfo.set(q, { product: p.제품군, rel, target: p.목표값 || 4 }); if (ids.has(p.안경사ID)) { if (!segByQ.has(q)) segByQ.set(q, []); segByQ.get(q).push(p.점수); } }); return [...segByQ.entries()].map(([q, vals]) => { const seg = avg(vals); const all = avg(allByQ.get(q) || []); const info = qInfo.get(q) || { rel: 0, target: 4, product: 'other' }; const diff = seg != null && all != null ? seg - all : null; const targetGap = seg != null ? seg - info.target : null; const severity = (diff == null ? 0 : Math.max(0, -diff) * 40) + (targetGap == null ? 0 : Math.max(0, -targetGap) * 25) + info.rel * 3; return { q, seg, all, diff, targetGap, rel: info.rel, product: info.product, severity }; }).filter(x => x.seg != null && (x.diff <= -0.2 || x.targetGap < 0 || x.rel >= 4)).sort((a, b) => b.severity - a.severity).slice(0, maxCount); }
  function educationTitle(row) { return clean(get(row, ['교육명', '콘텐츠명', '추천교육명', '과정명', 'Title'])) || clean(get(row, ['교육ID', '콘텐츠ID', 'ID'])); }
  function educationRelated(title, key) { const text = clean(title).toLowerCase(); return INSIGHT[key].keywords.some(k => text.includes(String(k).toLowerCase())); }
  function educationSummaryForRows(rows, key) { const ids = new Set(rows.map(r => r.안경사ID)); const all = S.edu.filter(r => ids.has(clean(get(r, ['안경사ID', '안경사 ID', 'ID'])))); const related = all.filter(r => educationRelated(Object.values(r).join(' '), key)); const base = related.length ? related : all; const done = base.filter(eduDone).length; const incomplete = base.length - done; const titles = [...new Set(base.map(educationTitle).filter(Boolean))].slice(0, 4); return { all, base, relatedCount: related.length, total: base.length, done, incomplete, rate: base.length ? done / base.length : null, titles }; }
  function completedEducationTitles(rows) { const ids = new Set(rows.map(r => r.안경사ID)); return new Set(S.edu.filter(r => ids.has(clean(get(r, ['안경사ID', '안경사 ID', 'ID']))) && eduDone(r)).map(educationTitle).filter(Boolean).map(norm)); }
  function pickEducationFromContent(key, completed) { const candidates = (S.content || []).map(r => ({ title: educationTitle(r), raw: r })).filter(x => x.title && educationRelated(x.title, key)); const notDone = candidates.filter(x => !completed.has(norm(x.title))).map(x => x.title); const all = candidates.map(x => x.title); const chosen = [...new Set([...notDone, ...all, ...INSIGHT[key].eduFallback])].slice(0, 2); while (chosen.length < 2) chosen.push(INSIGHT[key].eduFallback[chosen.length]); return chosen.slice(0, 2); }
  function recommendedEducationPlan(rows, key, primaryCause) { const done = completedEducationTitles(rows); const titles = pickEducationFromContent(key, done); const reason1 = primaryCause ? `${primaryCause.q} 문항의 선택 그룹 평균이 낮아 ${INSIGHT[key].focus} 보완이 필요합니다.` : `${INSIGHT[key].focus} 보완을 위한 기본 교육입니다.`; const lowScore = primaryCause && primaryCause.seg != null && primaryCause.seg < 3.6; return [{ step: '1차 추천', title: titles[0], reason: reason1 }, { step: '2차 추천', title: titles[1], reason: lowScore ? '인식 저하 폭이 커서 실전 스크립트·피팅 적용 교육을 이어서 권장합니다.' : '판매 전환을 위해 상담/피팅 실행 교육을 이어서 권장합니다.' }]; }
  ...
  function personHasLowRelatedPerception(personId, key) { const rows = S.perById.get(personId) || []; return rows.some(p => questionRelevance(p, key) > 0 && (p.gap || (p.점수 != null && p.목표값 != null && p.점수 < p.목표값))); }
  function personHasIncompleteRelatedEducation(personId, key) { const eduRows = S.eduById.get(personId) || []; const related = eduRows.filter(r => educationRelated(Object.values(r).join(' '), key)); const base = related.length ? related : eduRows; return base.some(r => !eduDone(r)); }
  function peopleInSalesAccounts(masterRows, salesRows) { const stores = new Set(dedupeSalesRows(salesRows).map(storeKey).filter(Boolean)); if (!stores.size) return []; return masterRows.filter(r => stores.has(storeKey(r))); }
  function affectedSalesRowsForProduct(salesRows, key, overallGrowth) { const rows = dedupeSalesRows(salesRows); const affected = rows.filter(r => { const g = growth([r], key); if (g == null) return false; const diff = overallGrowth != null ? g - overallGrowth : null; return g < 0 || (diff != null && diff <= -3); }); return affected.length ? affected : rows; }
  function educationTargetPeople(peopleRows, key) { const perceptionLow = peopleRows.filter(p => personHasLowRelatedPerception(p.안경사ID, key)); if (perceptionLow.length) return perceptionLow; const educationIncomplete = peopleRows.filter(p => personHasIncompleteRelatedEducation(p.안경사ID, key)); if (educationIncomplete.length) return educationIncomplete; return peopleRows; }
  function priorityScore(rg, diff, causeList) { const reverse = rg != null && rg < 0 ? Math.abs(rg) * 10 : 0; const underAvg = diff != null && diff < 0 ? Math.abs(diff) * 8 : 0; const perception = causeList && causeList.length ? causeList[0].severity : 0; return reverse + underAvg + perception; }
  function insight(type, title, rows, key, symptom, causeList, otherList, edu, recs, priority) { return { type, title, targetIds: rows.map(r => r.안경사ID), size: rows.length, key, symptom, causeList, otherList, edu, recs, priority }; }
  function generateInsights() { const out = []; const allSales = dedupeSalesRows(S.sales); const overall = { ast: growth(allSales, 'ast'), mf: growth(allSales, 'mf'), max: growth(allSales, 'max') }; const groups = []; ['Tier', '연차', '채널'].forEach(dim => { Object.entries(by(S.master, dim)).forEach(([value, rows]) => { if (rows.length >= 3) groups.push({ name: value, rows, dim }); }); }); groups.forEach(g => { const groupSales = selectedSalesRows(g.rows); if (!dedupeSalesRows(groupSales).length) return; ['ast', 'mf', 'max'].forEach(key => { const rg = growth(groupSales, key); if (rg == null) return; const diff = overall[key] != null ? rg - overall[key] : null; const avgPack = avgPackDeltaPerAcc(groupSales, key); const salesIssue = rg < 0 || (diff != null && diff <= -3); const affectedSales = affectedSalesRowsForProduct(groupSales, key, overall[key]); const affectedPeople = peopleInSalesAccounts(g.rows, affectedSales); const targetPeople = educationTargetPeople(affectedPeople, key); const insightPeople = targetPeople.length ? targetPeople : affectedPeople; if (!insightPeople.length) return; const causeList = lowQuestionsForRows(insightPeople, key, 3, true); const perceptionIssue = causeList.length > 0 && (causeList[0].diff <= -0.2 || causeList[0].targetGap < 0); if (!salesIssue && !perceptionIssue) return; const primary = causeList[0] || null; const otherList = otherLowAreas(insightPeople, key); const edu = educationSummaryForRows(insightPeople, key); const recs = recommendedEducationPlan(insightPeople, key, primary); const symptom = `${FITTING_COLUMNS[key].label} ${fmtPackPerAcc(avgPack)} (${fmtRate(rg)} vs PY)${diff != null ? ` / ${fmtPp(diff)} vs 전체평균` : ''}`; const priority = priorityScore(rg, diff, causeList); out.push(insight('판매 이상 → 개인 인식 원인 후보 → 교육 추천', `${g.name} ${FITTING_COLUMNS[key].label} 성장 이슈`, insightPeople, key, symptom, causeList, otherList, edu, recs, priority)); }); }); return out.sort((a, b) => b.priority - a.priority).slice(0, 5); }
  function renderInsightPlaceholder() { if ($('insightSummary')) { $('insightSummary').innerHTML = ''; $('insightSummary').style.display = 'none'; } if ($('insightCards')) $('insightCards').innerHTML = '<div class="empty-state">엑셀 업로드 후 <b>AI 인사이트 생성</b> 버튼을 누르세요.</div>'; }
  function causeHtml(list, key) { if (!list.length) return `${INSIGHT[key].focus}에서 통계적으로 두드러진 저하 문항은 아직 없습니다. 판매 실행, 상권, 제품 노출, 교육 이수 여부를 함께 점검하세요.`; return list.map(x => `<b>${esc(x.q)}</b><br><small>선택 그룹 평균 ${x.seg.toFixed(1)}점, 전체 평균 ${x.all.toFixed(1)}점${x.diff != null ? `, 차이 ${x.diff.toFixed(1)}점` : ''}</small>`).join('<hr>'); }
  ...
  function renderInsights() { S.insights = generateInsights(); if ($('insightSummary')) { $('insightSummary').innerHTML = ''; $('insightSummary').style.display = 'none'; } if (!$('insightCards')) return; $('insightCards').innerHTML = S.insights.length ? S.insights.map((item, idx) => `<div class="insight-card"><div class="type">${esc(item.type)}</div><h3>${idx + 1}. ${esc(item.title)}</h3><div class="insight-steps"><div class="insight-step"><small>1. 증상</small>${esc(item.symptom)}</div><div class="insight-step"><small>2. ${esc(INSIGHT[item.key].focus)} 원인 후보</small>${causeHtml(item.causeList, item.key)}</div><div class="insight-step"><small>3. 참고 저하 영역</small>${otherHtml(item.otherList)}</div><div class="insight-step"><small>4. 교육 이수 내역</small>${eduHtml(item.edu)}</div><div class="insight-step rec"><small>5. 추천 교육 2개</small>${recHtml(item.recs)}</div></div><div class="note">교육 대상 ${item.size}명</div><div class="insight-actions"><button class="button primary" data-insight="${idx}">대상 보기</button><button class="button" data-detail="${idx}">상세 보기</button></div></div>`).join('') : '<div class="empty-state">조건에 맞는 자동 인사이트가 없습니다.</div>'; document.querySelectorAll('[data-insight]').forEach(button => button.onclick = () => { const ins = S.insights[+button.dataset.insight]; S.targetIds = new Set(ins.targetIds); S.query = ''; render(); if ($('queryExplanation')) $('queryExplanation').textContent = `인사이트 대상 필터 적용: ${ins.title} / 결과 ${S.filtered.length}명`; view('segment'); }); document.querySelectorAll('[data-detail]').forEach(button => button.onclick = () => showInsightDetail(+button.dataset.detail)); }
  function showInsightDetail(idx) { const item = S.insights[idx]; if (!$('insightDetailPanel')) return; $('insightDetailPanel').hidden = false; $('insightDetail').innerHTML = `<div class="insight-card"><div class="type">${esc(item.type)}</div><h3>${esc(item.title)}</h3><div class="insight-steps"><div class="insight-step"><small>1. 증상</small>${esc(item.symptom)}</div><div class="insight-step"><small>2. ${esc(INSIGHT[item.key].focus)} 원인 후보</small>${causeHtml(item.causeList, item.key)}</div><div class="insight-step"><small>3. 참고 저하 영역</small>${otherHtml(item.otherList)}</div><div class="insight-step"><small>4. 교육 이수 내역</small>${eduHtml(item.edu)}</div><div class="insight-step rec"><small>5. 추천 교육 2개</small>${recHtml(item.recs)}</div></div><div class="note">교육 대상 ${item.size}명</div></div>`; $('insightDetailPanel').scrollIntoView({ behavior: 'smooth' }); }
