(function () {
  ...
  const INSIGHT = {
    ast: {
      focus: '난시 관련 인식',
      keywords: ['난시', '토릭', 'ASD', '조기교정', '교정', '피팅', '축', '원주'],
  ...
  function kpiGrowth(key, rows) {
    ...
    return kpi(
      ...
      `<span>${fmtRate(cur)} <span class="kpi-sub">(vs PY)</span></span><br><span class="delta ${dclass(diff)}">${fmtPp(diff)} <span class="kpi-sub">(vs 전체평균)</span></span>`
    );
  }
  ...
  function priorityScore(rg, diff, causeList) {
    // 화면에 표시하지 않는 내부 정렬 점수. 역성장 크기, 전체 평균 대비 부족분, 인식 저하 정도만 사용합니다.
  ...
  function generateInsights() {
    ...
    // 중요 순서: Tier → 연차 → 채널 → 담당영업사원 → 지역. 지역은 가장 마지막에 봅니다.
    ['Tier', '연차', '채널', '담당영업사원', '지역'].forEach(dim => {
    ...
    groups.forEach(g => {
      ...
      ['ast', 'mf', 'max'].forEach(key => {
        ...
        // 판매 이슈가 있거나, 제품군 관련 인식이 전체보다 눈에 띄게 낮으면 인사이트 후보로 봅니다.
