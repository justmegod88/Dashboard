...
const S={master:[],content:[],edu:[],qm:[],per:[],sales:[],rec:[],filtered:[],query:'',targetIds:null,insights:[]};
window.S=S;
...
const FITTING_COLUMNS={
  ast:{label:'난시 성장',py:['2025 난시 팩수','2025난시팩수','25년 난시 팩수','25년난시팩수'],cy:['2026 난시 팩수','2026난시팩수','26년 난시 팩수','26년난시팩수'],rate:['난시 성장률','난시성장률']},
  mf:{label:'멀티포컬 성장',py:['2025 멀티포컬  팩수','2025 멀티포컬 팩수','2025멀티포컬팩수','25년 멀티포컬  팩수','25년 멀티포컬 팩수','25년멀티포컬팩수'],cy:['2026 멀티포컬  팩수','2026 멀티포컬 팩수','2026멀티포컬팩수','26년 멀티포컬  팩수','26년 멀티포컬 팩수','26년멀티포컬팩수'],rate:['멀티포컬 성장률','멀티포컬성장률','MF 성장률','MF성장률']},
  max:{label:'MAX 성장',py:['2025 MAX  팩수','2025 MAX 팩수','2025MAX팩수','25년 MAX  팩수','25년 MAX 팩수','25년MAX팩수'],cy:['2026 MAX  팩수','2026 MAX 팩수','2026MAX팩수','26년 MAX  팩수','26년 MAX 팩수','26년MAX팩수'],rate:['MAX 성장률','MAX성장률','맥스 성장률','맥스성장률']}
};
...
function findWorkbookSheetName(wb,names){const sheetNames=wb.SheetNames||[];const targets=(names||[]).map(sheetNameKey).filter(Boolean);let n=sheetNames.find(s=>targets.includes(sheetNameKey(s)));if(n)return n;n=sheetNames.find(s=>targets.some(t=>sheetNameKey(s).includes(t)||t.includes(sheetNameKey(s))));if(n)return n;if((names||[]).some(x=>String(x).includes('피팅판매'))){n=sheetNames.find(s=>sheetNameKey(s).includes('피팅판매'));if(n)return n}return''}
function rowLooksLikeHeader(row){const text=(row||[]).map(v=>clean(v)).join('|');return [/안경사ID/.test(text),/안경원코드/.test(text),/안경원명/.test(text),/2025|25년/.test(text),/2026|26년/.test(text),/팩수/.test(text),/성장률/.test(text)].filter(Boolean).length>=3}
function aoaToObjects(aoa,idx){const h=(aoa[idx]||[]).map(x=>clean(x));const out=[];for(let i=idx+1;i<aoa.length;i++){const line=aoa[i]||[];if(!line.some(v=>clean(v)))continue;const o={};h.forEach((k,j)=>{if(k)o[k]=line[j]==null?'':line[j]});out.push(o)}return out}
function sheet(wb,names){const n=findWorkbookSheetName(wb,names);console.log('[sheet]',names,'=>',n);if(!n||!wb.Sheets[n])return[];const ws=wb.Sheets[n];let rows=XLSX.utils.sheet_to_json(ws,{defval:'',raw:true,blankrows:false});if(rows&&rows.length){console.log('[sheet loaded]',n,rows.length,rows[0]);return rows}const aoa=XLSX.utils.sheet_to_json(ws,{header:1,defval:'',raw:true,blankrows:false});const idx=aoa.findIndex(rowLooksLikeHeader);if(idx<0){console.warn('[sheet header not found]',n,aoa.slice(0,8));return[]}rows=aoaToObjects(aoa,idx);console.log('[sheet fallback loaded]',n,rows.length,rows[0]);return rows}
...
function packDelta(rows,key){const col=FITTING_COLUMNS[key];const py=sum(rows,r=>num(get(r,col.py)));const cy=sum(rows,r=>num(get(r,col.cy)));if(py==null&&cy==null)return null;return (cy||0)-(py||0)}
function growth(rows,key){const col=FITTING_COLUMNS[key];const direct=avg(rows,r=>num(get(r,col.rate)));if(direct!=null)return direct;const py=sum(rows,r=>num(get(r,col.py)));const cy=sum(rows,r=>num(get(r,col.cy)));if(py==null&&cy==null)return null;if(!py&&cy)return 100;return py?((cy-py)/py*100):null}
function fmtPack(v){return v==null?'데이터 없음':`${Math.round(Number(v))>=0?'+':''}${Math.round(Number(v)).toLocaleString('ko-KR')}팩`}
...
function kpiGrowth(key,rows){const ms=rows.map(r=>metrics(r.안경사ID));const cur=avg(ms,m=>m.growths[key].cur);const all=avg(S.master,r=>metrics(r.안경사ID).growths[key].cur);const d=cur!=null&&all!=null?cur-all:null;const pk=sum(ms,m=>m.growths[key].pack);return kpi(FITTING_COLUMNS[key].label,`<span class="${dclass(pk)}">${fmtPack(pk)}</span>`,`<span>${pp(cur)} <span class="kpi-sub">(vs PY)</span></span><br><span class="delta ${dclass(d)}">${pp(d)} <span class="kpi-sub">(vs 전체평균)</span></span>`)}
...
function renderQuestionTop(rows){const ids=new Set(rows.map(r=>r.안경사ID)),cnt={};S.per.forEach(p=>{if(ids.has(p.안경사ID)&&p.gap)cnt[p.문항]=(cnt[p.문항]||0)+1});$('questionTop').innerHTML=Object.entries(cnt).sort((a,b)=>b[1]-a[1]).slice(0,7).map((x,i)=>`<div class="rank-item"><span class="rank-no">${i+1}</span><b>${esc(x[0])}</b><span>${x[1]}명</span></div>`).join('')||'<div class="empty-state">인식 Gap 문항이 없습니다.</div>'}
...
function renderSegment(rows,ms){$('resultCount').textContent=`${rows.length.toLocaleString('ko-KR')}명`;$('segmentSummary').innerHTML=`<div class="three-col"><div>${kpiGrowth('ast',rows)}</div><div>${kpiGrowth('mf',rows)}</div><div>${kpiGrowth('max',rows)}</div></div>`;$('segmentTable').innerHTML=ms.map(m=>{const p=m.p||{};const eduName=clean(get(m.rec,['추천교육명','교육명']))||contentName(get(m.rec,['추천교육ID','교육ID']));return`<tr data-id="${esc(p.안경사ID)}"><td><b>${esc(p.안경사명)}</b><small><br>${esc(p.안경사ID)}</small></td><td>${esc(p.안경원명)}<small><br>${esc(p.지역)} · ${esc(p.채널)}</small></td><td>${esc(p.연차)} / ${esc(p.Tier)}</td><td>${m.eduRate==null?'데이터 없음':pct(m.eduRate)}</td><td>${m.gaps.length}개</td><td>${fmtPack(m.growths.ast.pack)}<br><small>${pp(m.growths.ast.cur)}</small></td><td>${fmtPack(m.growths.mf.pack)}<br><small>${pp(m.growths.mf.cur)}</small></td><td>${fmtPack(m.growths.max.pack)}<br><small>${pp(m.growths.max.cur)}</small></td><td>${esc(eduName||'없음')}</td><td>${m.priority}</td></tr>`}).join('');document.querySelectorAll('#segmentTable tr').forEach(tr=>tr.onclick=()=>showProfile(tr.dataset.id))}
...
function insight(type,title,rows,key,symptom,cause,education,action,score){
  return{type,title,targetIds:rows.map(r=>r.안경사ID),size:rows.length,key,symptom,cause,education,action,score};
}

function generateInsights(){
  const out=[], all=S.master;
  const overall={
    ast:avg(all,r=>metrics(r.안경사ID).growths.ast.cur),
    mf:avg(all,r=>metrics(r.안경사ID).growths.mf.cur),
    max:avg(all,r=>metrics(r.안경사ID).growths.max.cur)
  };
  const groups=[];
  ...
  groups.forEach(g=>{
    ...
    ['ast','mf','max'].forEach(key=>{
      const sg=avg(ms,m=>m.growths[key].cur), og=overall[key];
      if(sg==null)return;
      const diff=og!=null?sg-og:null;
      const belowOverall=diff!=null&&diff<0;
      const reverseGrowth=sg<0;
      if(!belowOverall&&!reverseGrowth)return;

      const qRelated=lowQuestions([key],ms);
      const qAll=lowQuestions('all',ms);
      const best=qRelated[0]||qAll[0];
      ...
      const eduText=edu.total
        ...
        : '교육 이력 데이터 없음';
      const symptomParts=[];
      if(reverseGrowth) symptomParts.push(`${P[key].label} 성장률이 전년 대비 ${rate(sg)}로 역성장입니다.`);
      if(belowOverall) symptomParts.push(`${P[key].label} 성장률이 전체 평균 대비 ${pp(diff)} 낮습니다.`);
      const cause=best
        ? `${best.q}: 선택 그룹 평균 ${best.seg.toFixed(1)}점, 전체 평균 ${best.all.toFixed(1)}점으로 ${best.diff.toFixed(1)}점 낮습니다.`
        : `${P[key].label} 관련 인식 문항의 뚜렷한 저하는 확인되지 않았습니다. 판매 실행, 상권, 제품 노출, 재고/프로모션 요인을 함께 점검하세요.`;
      const type=best?'판매 이상 → 인식 원인 후보':'판매 이상';
      const score=(reverseGrowth?20:0)+(belowOverall?Math.abs(diff)*6:0)+g.rows.length/3+(best?Math.abs(best.diff)*12:0)+(edu.rate!=null?(1-edu.rate)*10:0);
      out.push(insight(
        type,
        `${g.name} ${P[key].label} 성장 이슈`,
        g.rows,
        key,
        symptomParts.join(' '),
        cause,
  ...
  ['ast','mf','max'].forEach(key=>{
    const rows=all.filter(r=>metrics(r.안경사ID).gaps.some(g=>g.제품군===key));
    if(rows.length<3)return;
    const ms=rows.map(r=>metrics(r.안경사ID));
    const sg=avg(ms,m=>m.growths[key].cur), og=overall[key];
    if(sg==null)return;
    const diff=og!=null?sg-og:null;
    if(!(sg<0||(diff!=null&&diff<0)))return;
    const best=lowQuestions([key],ms)[0];
    const edu=educationSummaryForRows(rows,key);
    const eduText=edu.total
      ? `관련/전체 교육 이력 ${edu.total}건 중 완료 ${edu.done}건(${pct(edu.rate)}). ${edu.names.length?'주요 이수/추천 교육: '+edu.names.join(', '):'교육명 데이터 없음'}`
      : '교육 이력 데이터 없음';
    out.push(insight(
      '인식 Gap → 판매 이슈',
      `${P[key].label} 인식 Gap 보유 그룹`,
      rows,
      key,
      `${P[key].label} 인식 Gap 그룹의 성장률은 ${rate(sg)}이며 ${diff!=null?'전체 평균 대비 '+pp(diff)+'입니다.':'전체 평균 비교 데이터가 부족합니다.'}`,
      `${best?best.q:P[key].label+' 관련 문항'} Gap이 판매 이슈와 함께 관찰됩니다.`,
      eduText,
      P[key].edu,
      Math.abs(diff||0)*5+rows.length/4+(sg<0?15:0)+(edu.rate!=null?(1-edu.rate)*10:0)
    ));
  });

  return out.sort((a,b)=>b.score-a.score).slice(0,8);
}
function renderInsightPlaceholder(){S.insights=[];$('insightSummary').innerHTML=[kpi('인사이트 상태','대기','AI 인사이트 생성 버튼을 눌러 계산하세요')].join('');$('insightCards').innerHTML='<div class="empty-state">엑셀 업로드 후 <b>AI 인사이트 생성</b> 버튼을 누르면 판매 저하 → 인식 원인 → 교육 제안 순서로 자동 분석합니다.</div>'}
function renderInsights(){S.insights=generateInsights();$('insightSummary').innerHTML=[kpi('발견 인사이트',S.insights.length,'TOP 8'),kpi('우선 대상',S.insights.reduce((a,i)=>a+i.size,0).toLocaleString('ko-KR'),'중복 포함'),kpi('인식×판매',S.insights.filter(i=>/인식/.test(i.type)).length,'원인 후보'),kpi('교육 제안',S.insights.filter(i=>i.action).length,'자동 연결')].join('');$('insightCards').innerHTML=S.insights.length?S.insights.map((i,idx)=>`<div class="insight-card"><div class="type">${esc(i.type)}</div><h3>${idx+1}. ${esc(i.title)}</h3><div class="insight-steps"><div class="insight-step"><small>1. 증상</small>${esc(i.symptom)}</div><div class="insight-step"><small>2. 원인 후보</small>${esc(i.cause)}</div><div class="insight-step"><small>3. 교육 이력</small>${esc(i.education||'교육 이력 데이터 없음')}</div><div class="insight-step"><small>4. 추천 교육</small>${esc(i.action)}</div></div><div class="note">대상 ${i.size}명 · 점수 ${Math.round(i.score)}</div><div class="insight-actions"><button class="button primary" data-insight="${idx}">대상 보기</button><button class="button" data-detail="${idx}">상세 보기</button></div></div>`).join(''):'<div class="empty-state">아직 표시할 자동 인사이트가 없습니다. 엑셀 업로드 후 다시 확인하세요.</div>';document.querySelectorAll('[data-insight]').forEach(b=>b.onclick=()=>{const ins=S.insights[+b.dataset.insight];S.targetIds=new Set(ins.targetIds);S.query='';render();$('queryExplanation').textContent=`인사이트 대상 필터 적용: ${ins.title} / 결과 ${S.filtered.length}명`;view('segment')});document.querySelectorAll('[data-detail]').forEach(b=>b.onclick=()=>showInsightDetail(+b.dataset.detail))}
function showInsightDetail(idx){const i=S.insights[idx];$('insightDetailPanel').hidden=false;$('insightDetail').innerHTML=`<div class="insight-card"><div class="type">${esc(i.type)}</div><h3>${esc(i.title)}</h3><div class="insight-steps"><div class="insight-step"><small>1. 무엇이 낮은가</small>${esc(i.symptom)}</div><div class="insight-step"><small>2. 어떤 원인이 의심되는가</small>${esc(i.cause)}</div><div class="insight-step"><small>3. 교육 이력</small>${esc(i.education||'교육 이력 데이터 없음')}</div><div class="insight-step"><small>4. 어떤 교육을 추천할 것인가</small>${esc(i.action)}</div></div><p class="note">자동 탐색 결과입니다. 대상 규모가 작거나 데이터가 누락된 경우 해석에 주의하세요.</p></div>`;$('insightDetailPanel').scrollIntoView({behavior:'smooth'})}
...
function download(){const rows=S.filtered.map(p=>{const m=metrics(p.안경사ID);return{안경사ID:p.안경사ID,안경사명:p.안경사명,안경원명:p.안경원명,난시성장팩:m.growths.ast.pack,난시성장률:m.growths.ast.cur,멀티포컬성장팩:m.growths.mf.pack,멀티포컬성장률:m.growths.mf.cur,MAX성장팩:m.growths.max.pack,MAX성장률:m.growths.max.cur,인식Gap:m.gaps.length,교육완료율:m.eduRate}});const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(rows),'대상목록');XLSX.writeFile(wb,'ACUVUE_대상목록.xlsx')}
