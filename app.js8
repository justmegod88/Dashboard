(function () {
  ...
  function renderInsights() {
    ...
    box.querySelectorAll('[data-insight-target]').forEach(btn => {
      btn.onclick = () => {
        ...
        S.gapFilter = null;
        render();
        if ($('queryExplanation')) $('queryExplanation').textContent = `${item.title} 대상 ${item.targetPeople.length}명 · 판매 기준 ${S.baseMonth}월`;
        view('segment');
      };
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    $('runQuery').onclick = () => {
      S.query = $('smartQuery').value || ''; S.targetIds = null; S.gapFilter = null; render(); view('segment');
    };
    $('smartQuery').onkeydown = e => { if (e.key === 'Enter') $('runQuery').click(); };
    $('clearQuery').onclick = resetSmartSearch;
    $('resetFilters').onclick = resetFilters;
    document.querySelectorAll('.examples button').forEach(b => {
      b.onclick = () => { S.query = clean(b.dataset.query || b.textContent); $('smartQuery').value = S.query; S.targetIds = null; S.gapFilter = null; render(); view('segment'); };
    });
