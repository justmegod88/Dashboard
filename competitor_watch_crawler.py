...
import csv, datetime as dt, time, re, urllib.parse, traceback
...
def guess_region(text):
    return next((r for r in REGIONS if r in text), "미분류")
...
def activity_type(text):
    if "교육" in text or "세미나" in text: return "교육/세미나"
    if "체험" in text or "증정" in text: return "체험/증정"
    if "할인" in text or "이벤트" in text or "프로모션" in text: return "프로모션"
    return "기타"
    ...
    for page in range(1, pages + 1):
        ...
        try:
            ...
            for a in links[:10]:
                ...
                if not any(w in combined for w in EVENT_WORDS): continue
    ...
    try:
        ...
        for q in queries:
            for item in search_naver_view(q, pages=2):
                ...
                brand = guess_brand(text); region = guess_region(text)
                all_rows.append({
        ...
        rows = list({r["URL"]: r for r in all_rows}.values())
        write_csv(rows)
        print(f"Created output/Competitor_Activity.csv rows={len(rows)}")
    except Exception:
        print("Crawler failed but will create empty csv.")
        traceback.print_exc()
        write_csv([])
        raise SystemExit(0)
